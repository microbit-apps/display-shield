namespace shieldhelpers {
  ...
  export function wdsHandshake(wds_id: number, contact_attempts: number): boolean {
    ...
    const HANDSHAKE_FN_ID = 0;
    let buf = Buffer.fromArray([HANDSHAKE_FN_ID, wds_id])
    ...
    while (!handshakeRecieved) {
      ...
      if (attempts >= contact_attempts)
        break
    }
    radio.onReceivedString((_: string) => { })
    return handshakeRecieved;
  }

  export function wdsPresent(wds_id: number, contact_attempts: number): boolean {
    __present = undefined
    while (__present === undefined) {
      __present = wdsHandshake(wds_id, contact_attempts)
      simDisplayPresent()
      basic.pause(0)
    }
    return __present
