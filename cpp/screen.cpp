...
// this is a hack because someone (don't know where) #defined SPI to be
// NRF52SPI, which messes with the include file below the #undef
#undef SPI
...
public:
  ...
  WDisplay() {
    ...
    DMESG("configure screen: FRMCTR1=%p MADCTL=%p type=%d", frmctr1, madctl,
          dispTp);
...
void setPalette(Buffer buf) {
  ...
  for (int i = 0; i < 16; ++i) {
    display->currPalette[i] = (buf->data[i * 3] << 16) |
                              (buf->data[i * 3 + 1] << 8) |
                              (buf->data[i * 3 + 2] << 0);
